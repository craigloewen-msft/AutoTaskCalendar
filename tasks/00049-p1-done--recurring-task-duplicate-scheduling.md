...
| `scripts/port-allocator.js` | New. Probe-based free-block allocation with a cross-process lock |
| `scripts/dev.js` | Allocates ports, reports what it bound |
| `scripts/test.js` | Same allocator, so suite runs cannot collide |
| `instance.js` | Database name keyed on the name hash, not the port offset |
...
| `tests/api/dev-database.spec.js` | Specs for port allocation and database pinning |
