...
- A recommendation is an offer, not a silent assignment. It satisfies the gate only after the user clicks **Use suggestion** or chooses an option themselves.
...
Request a recommendation after the user has entered meaningful title text, using a short debounce and cancellation/stale-response protection. Do not call on every keystroke. Show at most one compact suggestion; if confidence is weak, show no recommendation and leave the ordinary selector available. Manual selection always wins and stops recommendation UI from changing the choice.
...
- Use a top-score and top-versus-runner-up margin to abstain. Return `null` rather than a weak or ambiguous suggestion; do not describe the score as a probability.
...
- invalidate only the affected user's model after task/project mutations that can change text or assignment.
...
- a relevant mutation invalidates the cached user model and the next result reflects the change;
...
- a returned suggestion is visible but is not selected until **Use suggestion** is clicked;
- a weak/failed recommendation leaves manual creation usable;
